const ejercicio2b = require('./ejercicio2b');
...
test('add the string "haxj210*x2&" that is match', () => {
...
test('add 18 years and 700 gains, this person does not to pay taxes', () => {
  ...
  test('do not add nothing "undefined" ', () => {
    expect(ejercicio2b.payTaxes()).toBe((console.error(`Error: You need add the age and gains.`))
    );
  });
  test('add a string "19" and 1400 gains ', () => {
    expect(ejercicio2b.payTaxes("19", 1400)).toBe(console.error(`Error: Both parameters must be numbers`));
  });
  test('add the string "Héctor" ', () => {
    expect(ejercicio2b.payTaxes("Héctor")).toBe(console.error(`Error: You need add the age and gains.`));
  });
